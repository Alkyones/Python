...
class TIC_TAC_TOE_AI:
    def __init__(self, root):
        ...
        self.window = root
        self.make_canvas = Canvas(self.window, background="#F39C12", relief=RAISED, bd=3)
    ...
    def decorating(self):# Basic Set-up
        Label(self.make_canvas, text="Tic-Tac-Toe AI", bg="#F39C12", fg="#fff", font=("Lato", 25, "bold")).place(x=110, y=10)
        ...
        self.machine_first_control = Button(self.make_canvas, text="Machine vs Human", font=("Arial", 15, "bold", "italic"), bg="#262626", activebackground="#262626", fg="#fff", relief=RAISED, bd=3, command=lambda: self.control_give("machine_first"))
        self.machine_first_control.place(x=15, y=380)

        self.human_first_control = Button(self.make_canvas, text="Human vs Machine", font=("Arial", 15, "bold", "italic"), bg="#262626", activebackground="#262626", fg="#fff", relief=RAISED, bd=3, command=self.control_give)
        self.human_first_control.place(x=240, y=380)

        self.reset_btn = Button(self.make_canvas, text="Reset", font=("Arial", 15, "bold", "italic"), bg="#262626", activebackground="#262626", disabledforeground="grey", fg="#fff", relief=RAISED, bd=3, command=self.reset, state=DISABLED)
    ...
    def __sign_insert(self, btn_indicator, sign_is="X"):# Button sign Insert
        if sign_is == "X":
            self.activate_btn[btn_indicator - 1].config(text=sign_is, state=DISABLED, disabledforeground="#F39C12")
            print(f"X chose : {btn_indicator }")
        else:
            self.activate_btn[btn_indicator - 1].config(text=sign_is, state=DISABLED, disabledforeground="white")
            print(f"O chose : {btn_indicator }")
...
if __name__ == "__main__":
    window = Tk()
    window.title("Tic-Tac-Toe Assignment")
